...
public class UndertowProxyDynamicConfig {

    @Autowired
    private LoadBalancingProxyClient userProxyClient;
    ...
    public WebServerFactoryCustomizer<UndertowServletWebServerFactory> undertowProxyCustomizer() {
        return factory -> factory.addDeploymentInfoCustomizers(deploymentInfo -> {
            deploymentInfo.addInitialHandlerChainWrapper(handler -> {
                ...
                // 配置代理路由
                HttpHandler userHandle = createProxyClient(userProxyClient);
    ...
    private HttpHandler createProxyClient(LoadBalancingProxyClient proxyClient) {
        try {
